...
export const sessions = {
  ...
  async complete(
    bookingId,
    elapsedSeconds,
    { rating, review, reason = "completed" } = {},
  ) {
  ...
  async list(status = null) {
    ...
    let query = supabase
      .from("bookings")
      .select(
        `id, skill_name, topic, scheduled_date, start_time, duration_mins, coin_rate, escrow_amount, status, actual_start_at, actual_end_at, learner_rating, reviewed_at, learner:learner_id(id, name, avatar_color), teacher:teacher_id(id, name, avatar_color)`,
      )
    ...
    const cancelled = processedData.filter((b) =>
      ["cancelled", "refunded", "partial_refund"].includes(b.status),
    );
  ...
  async getAll() {
    ...
    const mapper = (b) => ({
      ...
      color:
        b.teacher?.id === userId
          ? b.learner?.avatar_color || "#16A27B"
          : b.teacher?.avatar_color || "#5B45E0",
      skill: b.skill_name,
      ...
      rating: b.learner_rating || 5,
      reviewed: !!b.reviewed_at,
    });
    return {
