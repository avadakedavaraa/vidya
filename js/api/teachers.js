...
export const teachers = {
  ...
  async get(userId) {
    ...
    const { data, error } = await supabase
      .from("users")
      .select(
        `id, name, college, avatar_color, bio, xp, level, last_active_at, skills(id, name, category, tier, coin_rate, is_teaching)`,
      )
      .eq("id", userId)
      .eq("is_banned", false)
      .single();
  ...
  async myStudents() {
    ...
    const { data: sessions, error } = await supabase
      .from("bookings")
      .select(
        "id, status, skill_name, topic, scheduled_date, start_time, duration_mins, coin_rate, learner_rating, learner_review, reviewed_at, learner:learner_id(id, name, avatar_color)",
      )
  ...
  async myStats() {
    ...
    const { data: sessions } = await supabase
      .from("bookings")
      .select(
        "id, status, learner_rating, learner_review, coin_rate, duration_mins, scheduled_date, skill_name, learner:learner_id(id, name, avatar_color)",
      )
      .eq("teacher_id", user.id)
      .order("scheduled_date", { ascending: false });
    ...
    const ratings = completed
      .filter((s) => s.learner_rating)
      .map((s) => s.learner_rating);
    ...
    return {
      ...
      studentsCount: new Set(
        completed.map((s) => s.learner?.id).filter(Boolean),
      ).size,
  ...
  async getAll() {
    ...
    return list.map((t) => ({
      ...
      online: false,
      tier: t.skill_tier || t.tier || "unverified",
      teaches:
        skillsMap[t.id] && skillsMap[t.id].length
          ? skillsMap[t.id]
          : [t.skill_name || t.primary_skill].filter(Boolean),
      wants: [],
  ...
  async exploreStudents({ category = null, search = null } = {}) {
    ...
    let query = supabase
      .from("skills")
      .select(
        `id, name, category, created_at, users!inner(id, name, avatar_color, college, bio, xp, level)`,
      )
      .eq("is_learning", true)
      .neq("user_id", user.id)
  ...
  async getStats(userId) {
    ...
    const { data: sessions, error } = await supabase
      .from("bookings")
      .select(
        "status, learner_rating, learner_review, reviewed_at, learner:learner_id(name, avatar_color)",
      )
