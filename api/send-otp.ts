// api/send-otp.ts
// Generates a 6-digit OTP, stores the SHA-256 hash in DB, sends email via Resend.
...
export default async function (req: Request) {
  ...
  if (limited) return err('Too many OTP requests. Please wait 15 minutes.', req, 429);
  ...
  await db.from('otp_tokens').update({ used_at: new Date().toISOString() }).eq('email', email).eq('purpose', purpose).is('used_at', null);

  const { error: insertErr } = await db.from('otp_tokens').insert({ email, token_hash: hashHex, purpose });
  if (insertErr) return err('Could not generate OTP. Try again.', req, 500);

  const emailBody = buildEmailHTML(otp, purpose);
  const resendRes = await fetch('https://api.resend.com/emails', {
    method: 'POST',
    headers: { Authorization: `Bearer ${RESEND_API_KEY}`, 'Content-Type': 'application/json' },
    body: JSON.stringify({ from: FROM_EMAIL, to: [email], subject: getSubject(purpose), html: emailBody }),
  });

  if (!resendRes.ok) return err('Email delivery failed. Check your email address.', req, 500);
  return ok({ success: true, message: `OTP sent to ${email}` }, req);
}

function getSubject(purpose: string): string {
  const subjects: Record<string, string> = { signup: 'Your Vidyasetu verification code', login: 'Your Vidyasetu login code', reset_password: 'Reset your Vidyasetu password' };
  return subjects[purpose] ?? 'Your Vidyasetu code';
}

function buildEmailHTML(otp: string, purpose: string): string {
  const actionText = purpose === 'signup' ? 'complete your registration' : purpose === 'login' ? 'sign in to your account' : 'reset your password';
