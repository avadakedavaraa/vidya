...
export default async function (req: Request) {
  ...
  const tags        = Array.isArray(body.tags) ? (body.tags as string[]).map(t => sanitizeText(t, 40)).slice(0, 5) : [];
  ...
  const validCats = ['design','programming','content','data','marketing','language','music','other'];
  ...
  if (isNaN(deadline.getTime()) || deadline <= new Date()) return err('Deadline must be a future date', req, 400);

  const db = adminClient();
  const { data: balanceData } = await db.rpc('get_coin_balance', { p_user_id: user.id });
  if (Number(balanceData ?? 0) < coinReward) return err(`Insufficient coins. You have ${balanceData}, need ${coinReward}.`, req, 400);

  const { data: bounty, error: bountyErr } = await db.from('bounties').insert({
    poster_id: user.id, title, description, category, tags, coin_reward: coinReward, deadline_at: deadline.toISOString(), status: 'open',
  }).select('id, title, coin_reward').single();

  if (bountyErr || !bounty) return err('Failed to create bounty. Try again.', req, 500);

  try {
    await debitCoins(user.id, coinReward, 'bounty_posted', bounty.id, 'bounty', `Escrow for bounty: ${title.slice(0, 50)}`);
  } catch {
    await db.from('bounties').update({ status: 'cancelled' }).eq('id', bounty.id);
    return err('Could not lock coins. Please try again.', req, 500);
  }

  return ok({ success: true, bounty_id: bounty.id, title: bounty.title, coin_reward: bounty.coin_reward,
    message: `Bounty posted! 🪙 ${coinReward} coins locked in escrow.` }, req, 201);
}