...
// Called by Supabase scheduled function (cron) every 5 minutes.
// Finds sessions with stale heartbeats (>15 min) and triggers pro-rata release.
...
export default async function (req: Request) {
  // Security: only allow internal Supabase cron calls
  ...
  for (const dispute of oldDisputes ?? []) {
    try {
      ...
      // For booking disputes: 50/50 split of escrowed amount
  ...
  for (const bounty of expiredBounties ?? []) {
    try {
      ...
      // Refund escrow to poster if no bids were selected
      ...
      // Return deposits to all pending bidders
