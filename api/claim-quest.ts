...
export default async function (req: Request) {
  ...
  const { data: quest, error: questErr } = await db
    .from("quests")
    .select(
      "id, title, xp_reward, coin_reward, target, type, criteria, is_active",
    )
    .eq("id", questId)
    .single();
  ...
  if (!progress)
    return err("Quest progress not found. Complete the quest first.", req, 400);
  if (!progress.completed)
    return err(
      `Quest not yet complete. Progress: ${progress.progress}/${quest.target}`,
      req,
      400,
    );
  if (progress.claimed)
    return err("You have already claimed this quest reward.", req, 400);
  ...
  if (badgeSlug) {
    const cat =
      quest.criteria.includes("session") || quest.criteria.includes("teach")
        ? "teach"
        : quest.criteria.includes("bounty")
          ? "bounty"
          : quest.criteria.includes("gold")
            ? "teach"
            : "special";
    await db
      .from("user_badges")
      .upsert(
        { user_id: user.id, badge_slug: badgeSlug, category: cat },
