...
export default async function (req: Request) {
  ...
  if (elapsedSecs < 0)          return err('Elapsed seconds cannot be negative', req, 400);
  if (rating && (rating < 1 || rating > 5)) return err('Rating must be 1–5', req, 400);
  ...
  const { data: booking, error: bookingErr } = await db
    .from('bookings')
    .select('id, learner_id, teacher_id, escrow_amount, duration_mins, status, coin_rate, skill_name')
    .eq('id', bookingId)
    .single();
  ...
  if (!['confirmed', 'active'].includes(booking.status)) {
    return err(`Cannot complete session with status: ${booking.status}`, req, 400);
  }
  ...
  if (refundAmount > 0) {
    await creditCoins(
      ...
      bookingId,
      'booking',
      reason === 'trial_refund' ? '15-min trial refund' : 'Pro-rata refund (disconnect)'
    );
  }
...
async function incrementQuestProgress(
  ...
  try {
    ...
    for (const quest of quests) {
      ...
      if (existing) {
        await db.from('user_quest_progress').update({
